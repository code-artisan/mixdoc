import { defineConfig } from 'dumi';

export default defineConfig({
  title: 'mixdoc',
  favicon: 'data:image/png;base64,',
  logo: 'data:image/png;base64,',
  outputPath: 'dist',
  alias: {
    'react-native': 'react-native-web',
  },
  define: {
    __DEV__: process.env,
  },
  chainWebpack(memo) {
    memo.module
      .rule('jsx')
      .test(/\.js$/)
      .include.add(/react-native-animatable|react-native-reanimated|react-native-gesture-handler/)
      .end()
      .use('babel-loader')
      .loader('babel-loader')
      .options({
        presets: ['@babel/preset-env', '@babel/preset-react', '@babel/preset-flow'],
      })
      .end();
  },
  extraBabelPlugins: [
    '@babel/plugin-syntax-jsx',
    [
      'import',
      {
        libraryName: 'antd',
        style: true,
      },
      'antd',
    ],
    [
      'import',
      {
        libraryName: '@mixdoc/ui',
        style: true,
      },
      '@mixdoc/ui'
    ],
  ],
  sass: {
    implementation: require('node-sass'),
    sassOptions: {
      data: '@import "./.dumi/theme/styles/theme.scss";',
    },
  },
  // more config: https://d.umijs.org/config
});
